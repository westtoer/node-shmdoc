...
SchemaDoc.prototype.learnKeyValue = function (pathelms, value) {
    ...
    if (fd.learnable) { // ok no sweat - this is new stuff we don't know anything about
        fd.type = type;
        fd.minlen = (fd.minlen > len) ? len : fd.minlen;
        fd.maxlen = (fd.maxlen < len) ? len : fd.maxlen;
        fd.min = (fd.min > value) ? value : fd.min; //written like this will bootstrap if isNaN(fd.min) (like when undefined)
        fd.min = (fd.max < value) ? value : fd.max;
...
SchemaDoc.prototype.report = function (report) {
    ...
    this.refKeys.forEach(function (refKey) {
        ...
        if (isEmpty(tgtKey)) {
            return refFd.reference_errors.push("unknown tgt key");
        }
        tgtFd = this.base[tgtKey];
        if (isEmpty(tgtFd)) {
            return refFd.reference_errors.push("unknown tgt field definition");
        }
        ...
        if (isEmpty(tgtValues) || tgtValues.length < refValues.length) {
            return refFd.reference_errors.push("less available tgtvalues then used refvalues");
        }

        refValues.some(function (ref) {
            if (!contains(tgtValues, ref)) {
                refFd.reference_errors.push("unkown tgtValue : " + ref);
                return true;
            }
            return false;
        });
    ...
    //todo serialize base into path identified by report --> then remove sort and output above
};
