...
function handleJSONData(strm, cb) {
...
function handleCSVData(strm, cb) {
...
function handleData(type, strm, cb) {
    if (type === 'json' || type === 'application/json') {
        return handleJSONData(strm, cb);
    }
    if (type === 'csv' || type === 'text/csv') {
        return handleCSVData(strm, cb);
    }
...
SchemaDoc.prototype.loadBase = function (basePath, done) {
    ...
    if (basePath.indexOf('http') === 0) { // web reference to download base
        https.get(basePath, function (response) {
            handleData(response.headers['content-type'], response, onLoad);
        });
    } else { // assume file location locally
        if (fs.existsSync(basePath)) {
            handleData(path.extname(basePath).slice(1), fs.createReadStream(basePath, 'utf8'), onLoad);
        } else {
...
SchemaDoc.prototype.learnKeyValue = function (pathelms, value) {
    ...
    } else if (fd.type === "text" && value !== value.trim()) {
