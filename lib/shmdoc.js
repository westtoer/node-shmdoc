...
SchemaDoc.prototype.setBase = function (newbase) {
    ...
    Object.keys(this.base).forEach(function (k) {
        ...
        if (fd.isReferenceToKey) {
            ...
            if (me.base[fd.isReferenceToKey]) {
...
SchemaDoc.prototype.traverseValue = function (pathelms, d, cb) {
    ...
    if (d) {
        c = Object.prototype.toString.call(d.cloneNode);
        nt = d.nodeType;
    }

    if (c === "[object Function]"  && nt) { // this indicates the data-structure is a domNode
        /* nodetypes
            1=ELEMENT_NODE, 2=ATTRIBUTE_NODE, 3=TEXT_NODE, 4=CDATA_SECTION_NODE
            5=ENTITY_REFERENCE_NODE, 6=ENTITY_NODE, 7=PROCESSING_INSTRUCTION_NODE, 8=COMMENT_NODE,
            9=DOCUMENT_NODE, 10=DOCUMENT_TYPE_NODE, 11=DOCUMENT_FRAGMENT_NODE, 12=NOTATION_NODE
        */

        p = pathelms.slice(0);
        if (nt === 1) {  // nested elements

            //p.push(d.tagName.replace(':', '__'));
            p.push(d.localName);

            if (d.hasAttributes()) {
                for (nx = 0; nx < d.attributes.length; nx += 1) {
                    this.traverseValue(p, d.attributes[nx]);
                }
            }
            ...
            if (d.hasChildNodes()) {
                ...
                while (cn) {
                    if (cn.nodeType === 3 || cn.nodeType === 4) {
                        if (cn.nodeValue && cn.nodeValue.trim().length > 0) {
                            v += cn.nodeValue; // accumulate text and cdata children
                        }
                    } else { // TODO: dificulty in XML to recognise nested repeated values
                        this.traverseValue(p, cn);
                    }
                    cn = cn.nextSibling;
                }
                if (v.length > 0) {
                    this.traverseValue(p, v);
                }
            }

        } else if (nt === 2) { // attached attributes
            if (d.namespaceURI !== 'http://www.w3.org/2000/xmlns/'
                    && d.namespaceURI !== 'http://www.w3.org/2001/XMLSchema-instance') {

                //p.push("@" + d.nodeName);
                p.push(d.nodeName);

                this.traverseValue(p, d.nodeValue);
            }
        } else {
            console.log("unsupported xml node: @%j, type = %d, value = %s", p, nt, d.nodeValue);
...
SchemaDoc.prototype.learnFromXML = function (fPath, cb) {
    var me = this, data = "", error = false;

    //TODO check libxmljs support for stream support --> could be not-utf8 input!
    fs.createReadStream(fPath, { encoding: "utf-8" })
        .on('data', function (chunk) { if (!error) { data += chunk; } })
        .on('error', function (err) { if (err) { error = true; console.error(err); } })
        .on('end', function () {
            var doc = new DOMParser().parseFromString(data, 'text/xml'), root = doc.documentElement, cn;
            me.traverseValue([], root);
            if (cb) {
                cb();
            }
        });
...
SchemaDoc.prototype.learnFromCSV = function (fPath, cb) {
    ...
    fs.createReadStream(fPath, { encoding: "utf-8" })
        ...
        .pipe(csv.parse({ delimiter: delim, columns: true }))
        .pipe(csv.transform(
            function (data) {
                line += 1;
                lines.push(data);
...
SchemaDoc.learners = {
    ...
    xml: SchemaDoc.prototype.learnFromXML,
...
SchemaDoc.prototype.fieldDoc = function (pathelms) {
    ...
    this.base = this.base || {};
    var key, fd, cnt;
    pathelms = pathelms.slice(this.hideLevels);
    key = pathelms.join('.');
